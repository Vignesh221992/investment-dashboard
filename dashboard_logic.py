...
def resolve_sector(symbol):
    """Resolve a sector for a ticker using cacheable lookups."""
    symbol_upper = symbol.upper()
    if symbol_upper in MEGA_CAP_SECTORS:
        return MEGA_CAP_SECTORS[symbol_upper]
    try:
        ticker = yf.Ticker(symbol)
        return ticker.info.get("sector", "Financials/Other")
    except Exception:
        return "Technology"
    ...
    for label, ticker_symbol in symbols.items():
        try:
            ...
            if hist is not None and not hist.empty and len(hist) >= 2:
                ...
                daily_history = pd.Series([float(current_price)]).dropna() if current_price is not None else pd.Series([0.0])
...
def get_dynamic_allowed_universe():
    ...
    try:
        sp_df = pd.read_csv("https://raw.githubusercontent.com/datasets/s-and-p-500-companies/master/data/constituents.csv")
    ...
    try:
        ...
        if res.status_code == 200:
            allowed_tickers.update([line.strip().upper() for line in res.text.splitlines() if line.strip()])
    except Exception:
        pass
